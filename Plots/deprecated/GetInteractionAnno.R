...
WriteAnnovarInput <- function(){
  ...
  interaction_results <- dplyr::filter(interaction_results,rsid %in% colnames(readRDS('~/bsu_scratch/LDL_Project_Data_Aug2019/Random_Forest/rs12916_rs17238484_rs5909_rs2303152_rs10066707_rs2006760_LDLdirect/data_p_9e-06_maf_1e-2.rds')$dosageMatrix))
  ...
  data.table::fwrite(output,'~/bsu_scratch/LDL_Project_Data_Aug2019/Interaction_Data/annovar/HMGCR_int_indep_parsed',
                     col.names = F,sep='\t')
...
GetAllInteractionMetadata <- function(){
  ...
  file_name <- 'HMGCR_int_indep_parsed.variant_function'
  ...
  annovarResultFilt <- dplyr::left_join(annovarResultFilt,interaction_results)
  data.table::fwrite(annovarResultFilt,file='~/bsu_scratch/LDL_Project_Data_Aug2019/HMGCR_Interaction_Stats.csv',sep = ',')
  ...
  data.table::fwrite(geneRegionAnnovarUnique[order(geneRegionAnnovarUnique$`p-value`),],
                     file='~/bsu_scratch/LDL_Project_Data_Aug2019/HMGCR_Interaction_Stats_Indep.csv',sep = ',')
