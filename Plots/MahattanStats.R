...
data.table::fwrite(gene_snps %>% dplyr::filter(!is.na('P.value')),
                   paste0('~/bsu_scratch/Locus_Zoom/',geneName,'_SNPs.txt'),row.names = F,sep = '\t')

path <-  '~/bsu_scratch/LDL_Project_Data/Genotype_Data/'
bgen_file_prefix <- 'ukb_imp_chr#_HRConly'

refSNP <- gene_snps$MarkerName[which.min(gene_snps$`P.value`)]
...
data.table::fwrite(r2_df,file=paste0('~/bsu_scratch/Locus_Zoom/',geneName,'_r2.txt'),row.names = F,sep = ' ',quote=F)
